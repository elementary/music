/* PreferencesWindow.c generated by valac 0.11.7, the Vala compiler
...
struct _BeatBoxPreferencesWindowClass {
	...
	void (*fileOperationsDone) (BeatBoxPreferencesWindow* self);
};
...
void beat_box_preferences_window_fileOperationsDone (BeatBoxPreferencesWindow* self);
static void _beat_box_preferences_window_fileOperationsDone_beat_box_library_manager_file_operations_done (BeatBoxLibraryManager* _sender, gpointer self);
...
static void beat_box_preferences_window_real_fileOperationsDone (BeatBoxPreferencesWindow* self);
...
static void _beat_box_preferences_window_fileOperationsDone_beat_box_library_manager_file_operations_done (BeatBoxLibraryManager* _sender, gpointer self) {
	beat_box_preferences_window_fileOperationsDone (self);
}
...
BeatBoxPreferencesWindow* beat_box_preferences_window_construct (GType object_type, BeatBoxLibraryManager* lm, BeatBoxLibraryWindow* lw) {
	...
	g_signal_connect_object (self->priv->_lm, "file-operations-done", (GCallback) _beat_box_preferences_window_fileOperationsDone_beat_box_library_manager_file_operations_done, self, 0);
	return self;
}


BeatBoxPreferencesWindow* beat_box_preferences_window_new (BeatBoxLibraryManager* lm, BeatBoxLibraryWindow* lw) {
	return beat_box_preferences_window_construct (BEAT_BOX_TYPE_PREFERENCES_WINDOW, lm, lw);
}
...
void beat_box_preferences_window_buildUI (BeatBoxPreferencesWindow* self) {
	...
	gtk_window_set_transient_for ((GtkWindow*) self, (GtkWindow*) self->priv->_lw);
	gtk_widget_set_size_request ((GtkWidget*) self, 400, 300);
	g_object_set ((GtkWindow*) self, "allow-shrink", TRUE, NULL);
	_tmp0_ = gtk_widget_render_icon ((GtkWidget*) self, GTK_STOCK_PREFERENCES, GTK_ICON_SIZE_DIALOG, NULL);
	_tmp1_ = _tmp0_;
	gtk_window_set_icon ((GtkWindow*) self, _tmp1_);
	_g_object_unref0 (_tmp1_);
	_tmp2_ = (GtkVBox*) gtk_vbox_new (FALSE, 10);
	_g_object_unref0 (self->priv->content);
	self->priv->content = g_object_ref_sink (_tmp2_);
	_tmp3_ = (GtkHBox*) gtk_hbox_new (FALSE, 10);
	_g_object_unref0 (self->priv->padding);
	self->priv->padding = g_object_ref_sink (_tmp3_);
	_tmp4_ = (GtkLabel*) gtk_label_new ("Music Folder Location");
	_g_object_unref0 (self->priv->musicLabel);
	self->priv->musicLabel = g_object_ref_sink (_tmp4_);
	_tmp5_ = (GtkFileChooserButton*) gtk_file_chooser_button_new ("Music Folder", GTK_FILE_CHOOSER_ACTION_SELECT_FOLDER);
	_g_object_unref0 (self->priv->fileChooser);
	self->priv->fileChooser = g_object_ref_sink (_tmp5_);
	_tmp6_ = (GtkLabel*) gtk_label_new ("Library Management");
	_g_object_unref0 (self->priv->managementLabel);
	self->priv->managementLabel = g_object_ref_sink (_tmp6_);
	_tmp7_ = (GtkCheckButton*) gtk_check_button_new_with_label ("Keep Music folder organized");
	_g_object_unref0 (self->priv->organizeFolders);
	self->priv->organizeFolders = g_object_ref_sink (_tmp7_);
	_tmp8_ = (GtkCheckButton*) gtk_check_button_new_with_label ("Copy files to Music folder when added to Library");
	_g_object_unref0 (self->priv->copyImportedMusic);
	self->priv->copyImportedMusic = g_object_ref_sink (_tmp8_);
	_tmp9_ = (GtkLabel*) gtk_label_new ("Last FM Integration");
	_g_object_unref0 (self->priv->lastfmLabel);
	self->priv->lastfmLabel = g_object_ref_sink (_tmp9_);
	_tmp10_ = (GtkLabel*) gtk_label_new ("To allow for Last FM integration, you must give permission to BeatBox." \
" You only need to do this once.");
	_g_object_unref0 (self->priv->lastfmInfo);
	self->priv->lastfmInfo = g_object_ref_sink (_tmp10_);
	_tmp11_ = (GtkButton*) gtk_button_new_with_label ("Enable Scrobbling");
	_g_object_unref0 (self->priv->lastfmLogin);
	self->priv->lastfmLogin = g_object_ref_sink (_tmp11_);
	_tmp12_ = (GtkButton*) gtk_button_new_with_label ("Close");
	_g_object_unref0 (self->priv->saveChanges);
	self->priv->saveChanges = g_object_ref_sink (_tmp12_);
	...
	_tmp13_ = beat_box_settings_getMusicFolder (self->priv->_lm->settings);
	_tmp14_ = _tmp13_;
	gtk_file_chooser_set_current_folder ((GtkFileChooser*) self->priv->fileChooser, _tmp14_);
	_g_free0 (_tmp14_);
	if (self->priv->_lm->doing_file_operations) {
		gtk_widget_set_sensitive ((GtkWidget*) self->priv->fileChooser, FALSE);
		gtk_widget_set_tooltip_text ((GtkWidget*) self->priv->fileChooser, "You must wait until previous file operations finish before setting you" \
"r music folder");
	}
	_tmp15_ = beat_box_settings_getUpdateFolderHierarchy (self->priv->_lm->settings);
	gtk_toggle_button_set_active ((GtkToggleButton*) self->priv->organizeFolders, _tmp15_);
	_tmp16_ = beat_box_settings_getCopyImportedMusic (self->priv->_lm->settings);
	gtk_toggle_button_set_active ((GtkToggleButton*) self->priv->copyImportedMusic, _tmp16_);
	gtk_label_set_line_wrap (self->priv->lastfmInfo, TRUE);
	_tmp17_ = (GtkHButtonBox*) gtk_hbutton_box_new ();
	bottomButtons = g_object_ref_sink (_tmp17_);
	...
	_tmp18_ = beat_box_preferences_window_wrap_alignment ((GtkWidget*) self->priv->musicLabel, 10, 0, 0, 0);
	_tmp19_ = _tmp18_;
	gtk_box_pack_start ((GtkBox*) self->priv->content, (GtkWidget*) _tmp19_, FALSE, TRUE, (guint) 0);
	_g_object_unref0 (_tmp19_);
	_tmp20_ = beat_box_preferences_window_wrap_alignment ((GtkWidget*) self->priv->fileChooser, 0, 0, 0, 10);
	_tmp21_ = _tmp20_;
	gtk_box_pack_start ((GtkBox*) self->priv->content, (GtkWidget*) _tmp21_, FALSE, TRUE, (guint) 0);
	_g_object_unref0 (_tmp21_);
	gtk_box_pack_start ((GtkBox*) self->priv->content, (GtkWidget*) self->priv->managementLabel, FALSE, TRUE, (guint) 0);
	_tmp22_ = beat_box_preferences_window_wrap_alignment ((GtkWidget*) self->priv->organizeFolders, 0, 0, 0, 10);
	_tmp23_ = _tmp22_;
	gtk_box_pack_start ((GtkBox*) self->priv->content, (GtkWidget*) _tmp23_, FALSE, TRUE, (guint) 0);
	_g_object_unref0 (_tmp23_);
	_tmp24_ = beat_box_preferences_window_wrap_alignment ((GtkWidget*) self->priv->copyImportedMusic, 0, 0, 0, 10);
	_tmp25_ = _tmp24_;
	gtk_box_pack_start ((GtkBox*) self->priv->content, (GtkWidget*) _tmp25_, FALSE, TRUE, (guint) 0);
	_g_object_unref0 (_tmp25_);
	gtk_box_pack_start ((GtkBox*) self->priv->content, (GtkWidget*) self->priv->lastfmLabel, FALSE, TRUE, (guint) 0);
	_tmp26_ = beat_box_preferences_window_wrap_alignment ((GtkWidget*) self->priv->lastfmInfo, 0, 0, 0, 10);
	_tmp27_ = _tmp26_;
	gtk_box_pack_start ((GtkBox*) self->priv->content, (GtkWidget*) _tmp27_, FALSE, TRUE, (guint) 0);
	_g_object_unref0 (_tmp27_);
	_tmp28_ = beat_box_preferences_window_wrap_alignment ((GtkWidget*) self->priv->lastfmLogin, 0, 0, 0, 10);
	_tmp29_ = _tmp28_;
	gtk_box_pack_start ((GtkBox*) self->priv->content, (GtkWidget*) _tmp29_, FALSE, TRUE, (guint) 0);
...
static void beat_box_preferences_window_real_lastfmLoginClick (BeatBoxPreferencesWindow* self) {
	...
	if (_tmp0_) {
		...
		if (self->priv->lastfm_token == NULL) {
			...
			gchar* _tmp4_;
			gchar* auth_uri;
			_tmp4_ = g_strconcat ("http://www.last.fm/api/auth/?api_key=" LAST_FM_CORE_api "&token=", self->priv->lastfm_token, NULL);
			auth_uri = _tmp4_;
...
static void beat_box_preferences_window_real_fileOperationsDone (BeatBoxPreferencesWindow* self) {
	g_return_if_fail (self != NULL);
	gtk_widget_set_tooltip_text ((GtkWidget*) self->priv->fileChooser, "");
	gtk_widget_set_sensitive ((GtkWidget*) self->priv->fileChooser, TRUE);
}


void beat_box_preferences_window_fileOperationsDone (BeatBoxPreferencesWindow* self) {
	BEAT_BOX_PREFERENCES_WINDOW_GET_CLASS (self)->fileOperationsDone (self);
}
...
static void beat_box_preferences_window_class_init (BeatBoxPreferencesWindowClass * klass) {
	...
	BEAT_BOX_PREFERENCES_WINDOW_CLASS (klass)->fileOperationsDone = beat_box_preferences_window_real_fileOperationsDone;
