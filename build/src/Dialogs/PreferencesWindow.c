...
void beat_box_preferences_window_buildUI (BeatBoxPreferencesWindow* self) {
	...
	_tmp6_ = (GtkLabel*) gtk_label_new ("Music Folder Location");
	_tmp7_ = g_object_ref_sink (_tmp6_);
	_g_object_unref0 (self->priv->musicLabel);
	self->priv->musicLabel = _tmp7_;
	_tmp8_ = (GtkFileChooserButton*) gtk_file_chooser_button_new ("Music Folder", GTK_FILE_CHOOSER_ACTION_SELECT_FOLDER);
	_tmp9_ = g_object_ref_sink (_tmp8_);
	_g_object_unref0 (self->priv->fileChooser);
	self->priv->fileChooser = _tmp9_;
	_tmp10_ = (GtkLabel*) gtk_label_new ("Library Management");
	_tmp11_ = g_object_ref_sink (_tmp10_);
	_g_object_unref0 (self->priv->managementLabel);
	self->priv->managementLabel = _tmp11_;
	_tmp12_ = (GtkCheckButton*) gtk_check_button_new_with_label ("Keep Music folder organized");
	_tmp13_ = g_object_ref_sink (_tmp12_);
	_g_object_unref0 (self->priv->organizeFolders);
	self->priv->organizeFolders = _tmp13_;
	_tmp14_ = (GtkCheckButton*) gtk_check_button_new_with_label ("Copy files to Music folder when added to Library");
	_tmp15_ = g_object_ref_sink (_tmp14_);
	_g_object_unref0 (self->priv->copyImportedMusic);
	self->priv->copyImportedMusic = _tmp15_;
	_tmp16_ = (GtkLabel*) gtk_label_new ("Last FM Integration");
	_tmp17_ = g_object_ref_sink (_tmp16_);
	_g_object_unref0 (self->priv->lastfmLabel);
	self->priv->lastfmLabel = _tmp17_;
	_tmp18_ = (GtkLabel*) gtk_label_new ("To allow for Last FM integration, you must give permission to BeatBox." \
" You only need to do this once.");
	_tmp19_ = g_object_ref_sink (_tmp18_);
	_g_object_unref0 (self->priv->lastfmInfo);
	self->priv->lastfmInfo = _tmp19_;
	_tmp20_ = (GtkButton*) gtk_button_new_with_label ("Enable Scrobbling");
	_tmp21_ = g_object_ref_sink (_tmp20_);
	_g_object_unref0 (self->priv->lastfmLogin);
	self->priv->lastfmLogin = _tmp21_;
	_tmp22_ = (GtkButton*) gtk_button_new_with_label ("Close");
	_tmp23_ = g_object_ref_sink (_tmp22_);
	_g_object_unref0 (self->priv->saveChanges);
	self->priv->saveChanges = _tmp23_;
	...
	_tmp24_ = beat_box_settings_getMusicFolder (self->priv->_lm->settings);
	_tmp25_ = _tmp24_;
	gtk_file_chooser_set_current_folder ((GtkFileChooser*) self->priv->fileChooser, _tmp25_);
	_g_free0 (_tmp25_);
...
"r music folder");
	...
	_tmp26_ = beat_box_settings_getUpdateFolderHierarchy (self->priv->_lm->settings);
	gtk_toggle_button_set_active ((GtkToggleButton*) self->priv->organizeFolders, _tmp26_);
	_tmp27_ = beat_box_settings_getCopyImportedMusic (self->priv->_lm->settings);
	gtk_toggle_button_set_active ((GtkToggleButton*) self->priv->copyImportedMusic, _tmp27_);
	gtk_label_set_line_wrap (self->priv->lastfmInfo, TRUE);
	_tmp28_ = (GtkHButtonBox*) gtk_hbutton_box_new ();
	bottomButtons = g_object_ref_sink (_tmp28_);
	...
	_tmp29_ = beat_box_preferences_window_wrap_alignment ((GtkWidget*) self->priv->musicLabel, 10, 0, 0, 0);
	_tmp30_ = _tmp29_;
	gtk_box_pack_start ((GtkBox*) self->priv->content, (GtkWidget*) _tmp30_, FALSE, TRUE, (guint) 0);
	_g_object_unref0 (_tmp30_);
	_tmp31_ = beat_box_preferences_window_wrap_alignment ((GtkWidget*) self->priv->fileChooser, 0, 0, 0, 10);
	_tmp32_ = _tmp31_;
	...
	_tmp33_ = beat_box_preferences_window_wrap_alignment ((GtkWidget*) self->priv->organizeFolders, 0, 0, 0, 10);
	_tmp34_ = _tmp33_;
	gtk_box_pack_start ((GtkBox*) self->priv->content, (GtkWidget*) _tmp34_, FALSE, TRUE, (guint) 0);
	_g_object_unref0 (_tmp34_);
	_tmp35_ = beat_box_preferences_window_wrap_alignment ((GtkWidget*) self->priv->copyImportedMusic, 0, 0, 0, 10);
	_tmp36_ = _tmp35_;
	...
	_tmp37_ = beat_box_preferences_window_wrap_alignment ((GtkWidget*) self->priv->lastfmInfo, 0, 0, 0, 10);
	_tmp38_ = _tmp37_;
	gtk_box_pack_start ((GtkBox*) self->priv->content, (GtkWidget*) _tmp38_, FALSE, TRUE, (guint) 0);
	_g_object_unref0 (_tmp38_);
	_tmp39_ = beat_box_preferences_window_wrap_alignment ((GtkWidget*) self->priv->lastfmLogin, 0, 0, 0, 10);
	_tmp40_ = _tmp39_;
