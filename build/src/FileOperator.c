/* FileOperator.c generated by valac 0.11.5, the Vala compiler
...
static gboolean beat_box_file_operator_is_valid_file_type (BeatBoxFileOperator* self, const gchar* type) {
	...
	_tmp4_ = g_str_has_suffix (type, ".mp3");
	if (_tmp4_) {
		_tmp3_ = TRUE;
	} else {
		gboolean _tmp5_;
		_tmp5_ = g_str_has_suffix (type, ".m4a");
		_tmp3_ = _tmp5_;
	}
	if (_tmp3_) {
		_tmp2_ = TRUE;
		...
		gboolean _tmp6_;
		_tmp6_ = g_str_has_suffix (type, ".wma");
		_tmp2_ = _tmp6_;
	}
	if (_tmp2_) {
		_tmp1_ = TRUE;
		...
		gboolean _tmp7_;
		_tmp7_ = g_str_has_suffix (type, ".flac");
		_tmp1_ = _tmp7_;
	}
	if (_tmp1_) {
		_tmp0_ = TRUE;
		...
		gboolean _tmp8_;
		_tmp8_ = g_str_has_suffix (type, ".mp4");
		_tmp0_ = _tmp8_;
	}
	result = _tmp0_;
	return result;
...
void beat_box_file_operator_get_music_files (BeatBoxFileOperator* self, GFile* music_folder, GeeLinkedList** songs, GeeLinkedList** not_imported) {
	...
	while (TRUE) {
		...
		if (_tmp9_) {
			...
			g_signal_emit_by_name (self, "fo-progress", "", ((gdouble) ((gdouble) self->priv->index)) / ((gdouble) self->priv->item_count));
...
BeatBoxSong* beat_box_file_operator_import_song (BeatBoxFileOperator* self, const gchar* file_path) {
	...
	if (_tmp3_) {
		...
		fprintf (stdout, "Did not import %s.\n", file_path);
		result = NULL;
		_taglib_file_free0 (tag_file);
		_g_object_unref0 (s);
		return result;
...
gchar* beat_box_file_operator_find_album (BeatBoxFileOperator* self, BeatBoxSong* s) {
	...
	while (TRUE) {
		...
		if (_tmp13_ == G_FILE_TYPE_REGULAR) {
			const gchar* _tmp14_ = NULL;
			gboolean _tmp15_;
			_tmp14_ = g_file_info_get_name (file_info);
			_tmp15_ = beat_box_file_operator_is_image_file_type (self, _tmp14_);
			_tmp12_ = _tmp15_;
		} else {
			_tmp12_ = FALSE;
		}
...
GdkPixbuf* beat_box_file_operator_save_album (BeatBoxFileOperator* self, BeatBoxSong* s, const gchar* uri) {
	...
	filestream = _tmp4_;
	_tmp5_ = gdk_pixbuf_new_from_stream_at_scale ((GInputStream*) filestream, 200, 200, TRUE, NULL, &_inner_error_);
	_tmp6_ = _tmp5_;
	if (_inner_error_ != NULL) {
		goto __catch26_g_error;
	}
	...
	gdk_pixbuf_savev (rv, _tmp13_, "jpeg", &_tmp14_, &_tmp15_, &_inner_error_);
	_g_free0 (option_keys);
	_tmp16_ = g_strdup (_tmp14_);
	option_keys = _tmp16_;
	_g_free0 (option_values);
	_tmp17_ = g_strdup (_tmp15_);
	option_values = _tmp17_;
	_g_free0 (_tmp13_);
	_g_free0 (_tmp12_);
	_g_free0 (_tmp10_);
	if (_inner_error_ != NULL) {
...
GdkPixbuf* beat_box_file_operator_save_artist_image (BeatBoxFileOperator* self, BeatBoxSong* s, const gchar* uri) {
	...
	gdk_pixbuf_savev (rv, _tmp15_, "jpeg", &_tmp16_, &_tmp17_, &_inner_error_);
	_g_free0 (option_keys);
	_tmp18_ = g_strdup (_tmp16_);
	option_keys = _tmp18_;
	_g_free0 (option_values);
	_tmp19_ = g_strdup (_tmp17_);
	option_values = _tmp19_;
	_g_free0 (_tmp15_);
	_g_free0 (_tmp14_);
	_g_free0 (_tmp12_);
	_g_free0 (_tmp10_);
