/* LibraryManager.c generated by valac 0.11.7, the Vala compiler
...
GType beat_box_library_manager_shuffle_get_type (void) {
	...
	if (g_once_init_enter (&beat_box_library_manager_shuffle_type_id__volatile)) {
		static const GEnumValue values[] = {{BEAT_BOX_LIBRARY_MANAGER_SHUFFLE_OFF, "BEAT_BOX_LIBRARY_MANAGER_SHUFFLE_OFF", "off"}, {BEAT_BOX_LIBRARY_MANAGER_SHUFFLE_ALL, "BEAT_BOX_LIBRARY_MANAGER_SHUFFLE_ALL", "all"}, {0, NULL, NULL}};
...
void beat_box_library_manager_set_music_folder (BeatBoxLibraryManager* self, const gchar* folder) {
	...
	if (!self->doing_file_operations) {
		...
		_tmp1_ = _tmp0_;
		_tmp2_ = g_strconcat (_tmp1_, ". This may take a while", NULL);
		_tmp3_ = _tmp2_;
	...
	if (mode == BEAT_BOX_LIBRARY_MANAGER_SHUFFLE_OFF) {
		...
		if (mode == BEAT_BOX_LIBRARY_MANAGER_SHUFFLE_ALL) {
			...
			temp = _tmp4_;
			{
				...
				GeeIterator* _tmp8_;
				GeeIterator* _i_it;
				_tmp5_ = gee_map_get_values ((GeeMap*) self->priv->_current);
				_tmp6_ = _tmp5_;
			...
			}
			{
				...
				i = 0;
				{
					...
					while (TRUE) {
						...
						_tmp11_ = FALSE;
						_tmp12_ = gee_map_get_size ((GeeMap*) self->priv->_current);
						if (!(i < _tmp12_)) {
							break;
						}
						_tmp13_ = gee_collection_get_size ((GeeCollection*) temp);
						_tmp14_ = g_random_int_range ((gint32) 0, (gint32) _tmp13_);
						random = (gint) _tmp14_;
						_tmp15_ = gee_abstract_list_get ((GeeAbstractList*) temp, random);
						gee_abstract_map_set ((GeeAbstractMap*) self->priv->_current_shuffled, GINT_TO_POINTER (i), GINT_TO_POINTER (GPOINTER_TO_INT (_tmp15_)));
						_tmp16_ = gee_abstract_list_get ((GeeAbstractList*) temp, random);
						gee_abstract_collection_remove ((GeeAbstractCollection*) temp, GINT_TO_POINTER (GPOINTER_TO_INT (_tmp16_)));
					}
				}
			}
							...
							if (_tmp9_) {
								...
								if (_tmp24_) {
									while (TRUE) {
										...
										_tmp38_ = !(g_strcmp0 (_tmp36_, _tmp37_) == 0);
						...
						if (_tmp52_) {
							...
							if (_tmp54_) {
								while (TRUE) {
									...
									_tmp68_ = !(g_strcmp0 (_tmp66_, _tmp67_) == 0);
								...
								if (_tmp69_) {
									while (TRUE) {
										...
										_tmp83_ = !(g_strcmp0 (_tmp81_, _tmp82_) == 0);
...
void beat_box_library_manager_playSong (BeatBoxLibraryManager* self, gint id) {
	...
	_tmp4_ = beat_box_library_manager_song_from_id (self, id);
	_tmp5_ = _tmp4_;
	beat_box_settings_setLastSongPlaying (self->settings, _tmp5_);
	_g_object_unref0 (_tmp5_);
...
void beat_box_library_manager_save_album (BeatBoxLibraryManager* self, LastFMAlbumInfo* album) {
	...
	_tmp2_ = _tmp1_;
	_tmp3_ = last_fm_album_info_get_artist (album);
	_tmp4_ = g_strconcat (_tmp2_, _tmp3_, NULL);
	_tmp5_ = _tmp4_;
	gee_abstract_map_set ((GeeAbstractMap*) self->priv->_albums, _tmp5_, album);
	_g_free0 (_tmp5_);
	_g_free0 (_tmp2_);
}
...
void beat_box_library_manager_save_track (BeatBoxLibraryManager* self, LastFMTrackInfo* track) {
	...
	_tmp1_ = g_strconcat (_tmp0_, " by ", NULL);
	_tmp2_ = _tmp1_;
	_tmp3_ = last_fm_track_info_get_artist (track);
	_tmp4_ = g_strconcat (_tmp2_, _tmp3_, NULL);
	_tmp5_ = _tmp4_;
	gee_abstract_map_set ((GeeAbstractMap*) self->priv->_tracks, _tmp5_, track);
	_g_free0 (_tmp5_);
	_g_free0 (_tmp2_);
}
