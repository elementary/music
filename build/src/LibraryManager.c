/* LibraryManager.c generated by valac 0.11.5, the Vala compiler
...
void* beat_box_library_manager_set_music_thread_function (BeatBoxLibraryManager* self) {
	...
	_tmp0_ = beat_box_settings_getMusicFolders (self->settings);
	_tmp1_ = _tmp0_;
	_tmp2_ = g_file_new_for_path (_tmp1_);
	folders = (_tmp3_ = _tmp2_, _g_free0 (_tmp1_), _tmp3_);
	_tmp4_ = gee_linked_list_new (BEAT_BOX_TYPE_SONG, (GBoxedCopyFunc) g_object_ref, g_object_unref, NULL);
	new_songs = _tmp4_;
	_tmp5_ = gee_linked_list_new (G_TYPE_STRING, (GBoxedCopyFunc) g_strdup, g_free, NULL);
	_data2_->not_imported = _tmp5_;
	...
	index = 1;
	{
		...
		gint _s_size;
		gint _s_index;
		_tmp6_ = _g_object_ref0 (new_songs);
		_s_list = _tmp6_;
		...
		while (TRUE) {
			...
			_tmp9_ = index;
			index = _tmp9_ + 1;
			beat_box_song_set_rowid (s, _tmp9_);
			_tmp10_ = beat_box_song_get_rowid (s);
			gee_abstract_map_set ((GeeAbstractMap*) self->priv->_songs, GINT_TO_POINTER (_tmp10_), s);
			_g_object_unref0 (s);
		}
		_g_object_unref0 (_s_list);
	}
	{
		...
		gint _s_size;
		gint _s_index;
		_tmp11_ = _g_object_ref0 (_data2_->not_imported);
		_s_list = _tmp11_;
		...
		while (TRUE) {
			...
			s = (gchar*) _tmp13_;
			fprintf (stdout, "File %s was not imported\n", s);
			_g_free0 (s);
		}
		_g_object_unref0 (_s_list);
	}
...
static gboolean _lambda2_ (Block3Data* _data3_) {
	...
	gboolean result = FALSE;
	self = _data3_->self;
	g_signal_emit_by_name (self, "music-rescanned", _data3_->not_imported);
	result = FALSE;
	return result;
}
...
void* beat_box_library_manager_rescan_music_thread_function (BeatBoxLibraryManager* self) {
	...
	}
	{
		...
		GeeLinkedList* _folder_list;
		gint _tmp10_;
		gint _folder_size;
		gint _folder_index;
		_tmp9_ = beat_box_settings_getMusicFoldersList (self->settings);
		_folder_list = _tmp9_;
		_tmp10_ = gee_collection_get_size ((GeeCollection*) _folder_list);
		_folder_size = _tmp10_;
		_folder_index = -1;
		while (TRUE) {
			...
			if (!(_folder_index < _folder_size)) {
				break;
			}
			_tmp11_ = gee_abstract_list_get ((GeeAbstractList*) _folder_list, _folder_index);
			folder = (gchar*) _tmp11_;
			_tmp12_ = gee_collection_get_size ((GeeCollection*) paths);
			beat_box_file_operator_resetProgress (self->fo, _tmp12_);
			_tmp13_ = g_file_new_for_path (folder);
			_tmp14_ = _tmp13_;
			beat_box_file_operator_rescan_music (self->fo, _tmp14_, &paths, &_data3_->not_imported);
			_g_object_unref0 (_tmp14_);
			_g_free0 (folder);
		}
		_g_object_unref0 (_folder_list);
	}
	_tmp15_ = gee_linked_list_new (G_TYPE_INT, NULL, NULL, NULL);
	songs_to_remove = _tmp15_;
	{
		...
		while (TRUE) {
			...
			_tmp23_ = gee_abstract_collection_contains ((GeeAbstractCollection*) paths, _tmp22_);
			if (_tmp23_) {
				gint _tmp24_;
				_tmp24_ = beat_box_song_get_rowid (s);
				gee_abstract_collection_add ((GeeAbstractCollection*) songs_to_remove, GINT_TO_POINTER (_tmp24_));
			}
			_g_object_unref0 (s);
		}
		_g_object_unref0 (_s_it);
	}
	{
		...
		gint _id_size;
		gint _id_index;
		_tmp25_ = _g_object_ref0 (songs_to_remove);
		_id_list = _tmp25_;
		_tmp26_ = gee_collection_get_size ((GeeCollection*) _id_list);
		_id_size = _tmp26_;
		_id_index = -1;
		while (TRUE) {
			...
			_tmp27_ = gee_abstract_list_get ((GeeAbstractList*) _id_list, _id_index);
			id = GPOINTER_TO_INT (_tmp27_);
			beat_box_library_manager_remove_song_from_id (self, id);
		}
		_g_object_unref0 (_id_list);
	}
	g_idle_add_full (G_PRIORITY_DEFAULT_IDLE, __lambda2__gsource_func, block3_data_ref (_data3_), block3_data_unref);
	result = NULL;
	_g_object_unref0 (songs_to_remove);
...
void beat_box_library_manager_update_song (BeatBoxLibraryManager* self, BeatBoxSong* s) {
	...
	gee_abstract_map_set ((GeeAbstractMap*) self->priv->_songs, GINT_TO_POINTER (_tmp0_), s);
	beat_box_file_operator_save_song (self->fo, s);
	_tmp1_ = beat_box_settings_getUpdateFolderHierarchy (self->settings);
	if (_tmp1_) {
		beat_box_file_operator_update_file_hierarchy (self->fo, s);
	}
	_tmp2_ = beat_box_song_get_rowid (s);
	g_signal_emit_by_name (self, "song-updated", _tmp2_);
}
...
void beat_box_library_manager_playSong (BeatBoxLibraryManager* self, gint id) {
	...
	_tmp1_ = _tmp0_;
	beat_box_stream_player_play_song (self->player, _tmp1_);
	_g_object_unref0 (_tmp1_);
	_tmp2_ = beat_box_library_manager_song_from_id (self, id);
	_tmp3_ = _tmp2_;
	_g_object_unref0 (self->song_info->song);
	self->song_info->song = _tmp3_;
	if (!self->playing) {
		beat_box_stream_player_pause_stream (self->player);
	}
	g_signal_emit_by_name (self, "song-played", id);
}
...
static void beat_box_library_manager_class_init (BeatBoxLibraryManagerClass * klass) {
	...
	g_signal_new ("song_updated", BEAT_BOX_TYPE_LIBRARY_MANAGER, G_SIGNAL_RUN_LAST, 0, NULL, NULL, g_cclosure_marshal_VOID__INT, G_TYPE_NONE, 1, G_TYPE_INT);
	...
	g_signal_new ("song_played", BEAT_BOX_TYPE_LIBRARY_MANAGER, G_SIGNAL_RUN_LAST, 0, NULL, NULL, g_cclosure_marshal_VOID__INT, G_TYPE_NONE, 1, G_TYPE_INT);
}
