/* SmartPlaylist.c generated by valac 0.11.7, the Vala compiler
...
BeatBoxSmartPlaylist* beat_box_smart_playlist_construct (GType object_type) {
	...
	self->query_count = 0;
	_tmp3_ = gee_array_list_new (BEAT_BOX_TYPE_SMART_QUERY, (GBoxedCopyFunc) g_object_ref, g_object_unref, NULL);
	_g_object_unref0 (self->priv->_queries);
	self->priv->_queries = _tmp3_;
...
gchar* beat_box_smart_playlist_queries_to_string (BeatBoxSmartPlaylist* self) {
	...
	rv = _tmp0_;
	{
		...
		while (TRUE) {
			...
			_tmp6_ = _tmp5_;
			_tmp7_ = beat_box_smart_query_get_comparator (q);
			_tmp8_ = g_strconcat (_tmp6_, _tmp7_, NULL);
			_tmp9_ = _tmp8_;
			_tmp10_ = g_strconcat (_tmp9_, "<value_seperator>", NULL);
			_tmp11_ = _tmp10_;
			_tmp12_ = beat_box_smart_query_get_value (q);
			_tmp13_ = g_strconcat (_tmp11_, _tmp12_, NULL);
			_tmp14_ = _tmp13_;
			_tmp15_ = g_strconcat (_tmp14_, "<query_seperator>", NULL);
			_tmp16_ = _tmp15_;
										...
										if (g_strcmp0 (_tmp162_, "Length") == 0) {
											...
											if (g_strcmp0 (_tmp175_, "Rating") == 0) {
												...
												if (g_strcmp0 (_tmp188_, "Date Added") == 0) {
													...
													played = _tmp194_;
													_tmp195_ = beat_box_smart_query_get_comparator (q);
													if (g_strcmp0 (_tmp195_, "is exactly") == 0) {
														gboolean _tmp196_ = FALSE;
														gint _tmp197_;
														gint _tmp198_;
														_tmp197_ = g_date_time_get_day_of_year (now);
														_tmp198_ = g_date_time_get_day_of_year (played);
														if (_tmp197_ == _tmp198_) {
															gint _tmp199_;
															gint _tmp200_;
															_tmp199_ = g_date_time_get_year (now);
															_tmp200_ = g_date_time_get_year (played);
														...
														_tmp201_ = beat_box_smart_query_get_comparator (q);
														if (g_strcmp0 (_tmp201_, "is within") == 0) {
															gint _tmp202_;
															_tmp202_ = g_date_time_compare (played, now);
															result = _tmp202_ > 0;
															_g_date_time_unref0 (played);
															_g_date_time_unref0 (now);
															return result;
															...
															_tmp203_ = beat_box_smart_query_get_comparator (q);
															if (g_strcmp0 (_tmp203_, "is before") == 0) {
																gint _tmp204_;
																_tmp204_ = g_date_time_compare (now, played);
																result = _tmp204_ > 0;
																_g_date_time_unref0 (played);
																_g_date_time_unref0 (now);
																return result;
													...
													_tmp205_ = beat_box_smart_query_get_field (q);
													if (g_strcmp0 (_tmp205_, "Last Played") == 0) {
														gint _tmp206_;
														...
														_tmp206_ = beat_box_song_get_last_played (s);
														if (_tmp206_ == 0) {
															result = FALSE;
															return result;
														}
														_tmp207_ = g_date_time_new_now_local ();
														now = _tmp207_;
														_tmp208_ = beat_box_song_get_last_played (s);
														_tmp209_ = g_date_time_new_from_unix_local ((gint64) _tmp208_);
														played = _tmp209_;
														_tmp210_ = beat_box_smart_query_get_value (q);
														_tmp211_ = atoi (_tmp210_);
														_tmp212_ = g_date_time_add_days (played, _tmp211_);
														_g_date_time_unref0 (played);
														played = _tmp212_;
														_tmp213_ = beat_box_smart_query_get_comparator (q);
														if (g_strcmp0 (_tmp213_, "is exactly") == 0) {
															gboolean _tmp214_ = FALSE;
															gint _tmp215_;
															gint _tmp216_;
															_tmp215_ = g_date_time_get_day_of_year (now);
															_tmp216_ = g_date_time_get_day_of_year (played);
															if (_tmp215_ == _tmp216_) {
																gint _tmp217_;
																gint _tmp218_;
																_tmp217_ = g_date_time_get_year (now);
																_tmp218_ = g_date_time_get_year (played);
																_tmp214_ = _tmp217_ == _tmp218_;
															} else {
																_tmp214_ = FALSE;
															}
															result = _tmp214_;
															...
															_tmp219_ = beat_box_smart_query_get_comparator (q);
															if (g_strcmp0 (_tmp219_, "is within") == 0) {
																gint _tmp220_;
																_tmp220_ = g_date_time_compare (played, now);
																result = _tmp220_ > 0;
																_g_date_time_unref0 (played);
																_g_date_time_unref0 (now);
																return result;
																...
																if (g_strcmp0 (_tmp221_, "is before") == 0) {
																	gint _tmp222_;
																	_tmp222_ = g_date_time_compare (now, played);
																	result = _tmp222_ > 0;
																	_g_date_time_unref0 (played);
																	_g_date_time_unref0 (now);
																	return result;
