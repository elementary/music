/* RatingWidget.c generated by valac 0.11.7, the Vala compiler
...
BeatBoxRatingWidget* beat_box_rating_widget_construct (GType object_type) {
	...
	self->priv->starred = _tmp1_;
	_tmp2_ = gtk_widget_render_icon ((GtkWidget*) self, "not-starred", GTK_ICON_SIZE_SMALL_TOOLBAR, NULL);
	_g_object_unref0 (self->priv->not_starred);
	self->priv->not_starred = _tmp2_;
	_tmp3_ = gdk_pixbuf_get_width (self->priv->starred);
	g_object_set ((GtkWidget*) self, "width-request", _tmp3_ * 5, NULL);
	_tmp4_ = gdk_pixbuf_get_height (self->priv->starred);
	g_object_set ((GtkWidget*) self, "height-request", _tmp4_, NULL);
	g_object_get ((GtkWidget*) self, "width-request", &_tmp5_, NULL);
	g_object_get ((GtkWidget*) self, "height-request", &_tmp6_, NULL);
	_tmp7_ = gdk_pixbuf_new (GDK_COLORSPACE_RGB, TRUE, 8, _tmp5_, _tmp6_);
	_g_object_unref0 (self->priv->_canvas);
	self->priv->_canvas = _tmp7_;
...
static gboolean beat_box_rating_widget_real_motion_notify_event (GtkWidget* base, GdkEventMotion* event) {
	...
	buffer = (((GtkWidget*) self)->allocation.width - _tmp0_) / 2;
	if (((*event).x - buffer) > 5) {
...
static gboolean beat_box_rating_widget_real_exposeEvent (BeatBoxRatingWidget* self, GdkEventExpose* event) {
	...
	g_object_get ((GtkWidget*) self, "width-request", &_tmp1_, NULL);
	g_object_get ((GtkWidget*) self, "width-request", &_tmp2_, NULL);
	g_object_get ((GtkWidget*) self, "height-request", &_tmp3_, NULL);
	gdk_draw_pixbuf ((GdkDrawable*) (*event).window, _tmp0_->bg_gc[0], self->priv->_canvas, 0, 0, ((*event).area.width - _tmp1_) / 2, 0, _tmp2_, _tmp3_, GDK_RGB_DITHER_NONE, 0, 0);
	result = TRUE;
	return result;
}
