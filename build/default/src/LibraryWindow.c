...
static gboolean _lambda20_ (Block5Data* _data5_);
...
BeatBoxLibraryWindow* beat_box_library_window_construct (GType object_type, BeatBoxDataBaseManager* dbm, BeatBoxStreamPlayer* player) {
	...
	if (_tmp12_) {
		...
		if (_tmp17_) {
			...
			_data4_->s = _tmp26_;
			_tmp27_ = beat_box_song_get_title (_data4_->s);
			_tmp28_ = beat_box_song_get_artist (_data4_->s);
			_tmp29_ = beat_box_library_manager_song_from_name (self->priv->lm, _tmp27_, _tmp28_);
			_tmp30_ = _tmp29_;
			_g_object_unref0 (_data4_->s);
			_data4_->s = _tmp30_;
			_tmp31_ = beat_box_song_get_rowid (_data4_->s);
			if (_tmp31_ != 0) {
				Block5Data* _data5_;
				gint _tmp32_;
				...
				_data5_->position = (gint) _tmp32_;
				g_timeout_add_full (G_PRIORITY_DEFAULT, (guint) 500, __lambda20__gsource_func, block5_data_ref (_data5_), block5_data_unref);
...
void beat_box_library_window_build_ui (BeatBoxLibraryWindow* self) {
	...
	((GtkPaned*) self->priv->songsToInfo)->child1_resize = (guint) 1;
	...
	gtk_paned_add1 ((GtkPaned*) self->priv->songsToInfo, (GtkWidget*) self->priv->contentBox);
	gtk_paned_add2 ((GtkPaned*) self->priv->songsToInfo, (GtkWidget*) self->priv->infoPanel);
	gtk_paned_add1 ((GtkPaned*) self->priv->sourcesToSongs, (GtkWidget*) self->priv->sideBar);
	gtk_paned_add2 ((GtkPaned*) self->priv->sourcesToSongs, (GtkWidget*) self->priv->songsToInfo);
...
static void beat_box_library_window_real_song_played (BeatBoxLibraryWindow* self, gint i) {
	...
	beat_box_library_window_updateSongInfo (self);
	_g_free0 (song_label);
}
...
void* beat_box_library_window_lastfm_thread_function (BeatBoxLibraryWindow* self) {
	...
	if (g_strcmp0 (_tmp44_, _tmp45_) != 0) {
		...
		if (!_tmp47_) {
			...
			_tmp53_ = beat_box_library_manager_get_artist_image_location (self->priv->lm, _tmp52_);
			_tmp54_ = _tmp53_;
...
static void beat_box_library_window_real_helpAboutClick (BeatBoxLibraryWindow* self) {
	...
	_tmp2_ = _tmp1_;
	gtk_window_set_icon ((GtkWindow*) _data6_->ad, _tmp2_);
	_g_object_unref0 (_tmp2_);
	_tmp3_ = gtk_widget_render_icon ((GtkWidget*) self, "beatbox", GTK_ICON_SIZE_DIALOG, NULL);
	_tmp4_ = _tmp3_;
	gtk_about_dialog_set_logo (_data6_->ad, _tmp4_);
		...
		while (TRUE) {
			...
			if (_tmp8_ != 0) {
				...
				if (_tmp9_ == _tmp10_) {
					...
					gee_deque_offer_head ((GeeDeque*) similarIDs, GINT_TO_POINTER (_tmp11_));
				} else {
					gint _tmp12_;
					_tmp12_ = beat_box_song_get_rowid (s);
					gee_abstract_collection_add ((GeeAbstractCollection*) similarIDs, GINT_TO_POINTER (_tmp12_));
				}
			} else {
