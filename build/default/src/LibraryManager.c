...
static void block2_data_unref (Block2Data* _data2_) {
	if (g_atomic_int_dec_and_test (&_data2_->_ref_count_)) {
		...
		_g_object_unref0 (_data2_->new_songs);
...
static gboolean _lambda3_ (Block2Data* _data2_) {
	...
	self = _data2_->self;
	beat_box_library_manager_save_songs (self);
	g_signal_emit_by_name (self, "music-imported", _data2_->new_songs, _data2_->not_imported);
	result = FALSE;
	return result;
}
...
void* beat_box_library_manager_add_folder_to_library_thread (BeatBoxLibraryManager* self) {
	...
	_data2_->new_songs = _tmp2_;
	_tmp3_ = gee_linked_list_new (G_TYPE_STRING, (GBoxedCopyFunc) g_strdup, g_free, NULL);
	_data2_->not_imported = _tmp3_;
	beat_box_file_operator_get_music_files (self->fo, file, &_data2_->new_songs, &_data2_->not_imported);
	index = 1;
	{
		GeeSet* _tmp4_ = NULL;
		GeeSet* _tmp5_;
	...
	_tmp10_ = gee_collection_get_size ((GeeCollection*) _data2_->new_songs);
	...
	progress = 0;
	{
		...
		gint _s_size;
		gint _s_index;
		_tmp11_ = _g_object_ref0 (_data2_->new_songs);
		_s_list = _tmp11_;
		...
		while (TRUE) {
			...
			progress++;
			_tmp18_ = gee_collection_get_size ((GeeCollection*) _data2_->new_songs);
...
void* beat_box_library_manager_rescan_music_thread_function (BeatBoxLibraryManager* self) {
	...
	}
	{
		...
		gint _s_size;
		gint _s_index;
		_tmp28_ = _g_object_ref0 (_data3_->new_songs);
		_s_list = _tmp28_;
