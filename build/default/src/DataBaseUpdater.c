/* DataBaseUpdater.c generated by valac 0.11.6, the Vala compiler
...
BeatBoxDataBaseUpdater* beat_box_data_base_updater_construct (GType object_type, BeatBoxDataBaseManager* databm) {
	...
	self->priv->toUpdate = _tmp3_;
	_tmp4_ = gee_linked_list_new (G_TYPE_OBJECT, (GBoxedCopyFunc) g_object_ref, g_object_unref, NULL);
	_tmp5_ = _tmp4_;
	_g_object_unref0 (self->priv->toRemove);
	self->priv->toRemove = _tmp5_;
...
void* beat_box_data_base_updater_update_db_thread_function (BeatBoxDataBaseUpdater* self) {
	...
	while (TRUE) {
		...
		if (next != NULL) {
			...
			GObject* _tmp3_;
			_tmp2_ = gee_queue_poll ((GeeQueue*) self->priv->toRemove);
