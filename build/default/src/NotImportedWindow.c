...
static void _lambda18_ (Block7Data* _data7_);
...
static void _lambda18_ (Block7Data* _data7_) {
	...
	gboolean _tmp0_;
	self = _data7_->self;
	_tmp0_ = gtk_expander_get_expanded (_data7_->exp);
	if (_tmp0_) {
...
BeatBoxNotImportedWindow* beat_box_not_imported_window_construct (GType object_type, GeeLinkedList* files) {
	...
	_data7_->self = g_object_ref (self = (BeatBoxNotImportedWindow*) g_object_new (object_type, NULL));
	...
	_data7_->exp = g_object_ref_sink (_tmp48_);
	gtk_container_add ((GtkContainer*) _data7_->exp, (GtkWidget*) listBox);
	gtk_expander_set_expanded (_data7_->exp, FALSE);
	...
	_tmp52_ = beat_box_not_imported_window_wrap_alignment ((GtkWidget*) _data7_->exp, 0, 0, 0, 75);
	_tmp53_ = _tmp52_;
	...
	g_signal_connect_data (_data7_->exp, "activate", (GCallback) __lambda18__gtk_expander_activate, block7_data_ref (_data7_), (GClosureNotify) block7_data_unref, 0);
