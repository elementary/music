/* BeatBox.c generated by valac 0.11.7, the Vala compiler
...
gint beat_box_beatbox_main (gchar** args, int args_length1) {
	...
	app = _tmp0_;
	g_object_get (app, "is-running", &_tmp1_, NULL);
	if (_tmp1_) {
		...
		BeatBoxDataBaseManager* _tmp4_ = NULL;
		BeatBoxLibraryWindow* _tmp5_ = NULL;
		gdk_threads_init ();
		...
		beat_box_beatbox__player = _tmp3_;
		fprintf (stdout, "Creating database manager\n");
		_tmp4_ = beat_box_data_base_manager_new (TRUE, TRUE);
		_g_object_unref0 (beat_box_beatbox_dbm);
		beat_box_beatbox_dbm = _tmp4_;
		fprintf (stdout, "Loading database\n");
		beat_box_data_base_manager_load_db (beat_box_beatbox_dbm);
		_tmp5_ = beat_box_library_window_new (beat_box_beatbox_dbm, beat_box_beatbox__player);
		_g_object_unref0 (beat_box_beatbox__program);
		beat_box_beatbox__program = g_object_ref_sink (_tmp5_);
