/* SideTreeView.c generated by valac 0.11.6, the Vala compiler
...
void beat_box_side_tree_view_buildUI (BeatBoxSideTreeView* self) {
	...
	cell_renderer_text = g_object_ref_sink (_tmp5_);
	_tmp6_ = (GtkCellRendererPixbuf*) gtk_cell_renderer_pixbuf_new ();
	cell_renderer_pixbuf = g_object_ref_sink (_tmp6_);
	...
	gtk_cell_layout_pack_end ((GtkCellLayout*) _tmp7_, (GtkCellRenderer*) cell_renderer_text, TRUE);
	_tmp8_ = gtk_tree_view_get_column ((GtkTreeView*) self, 2);
	gtk_tree_view_column_set_attributes (_tmp8_, (GtkCellRenderer*) cell_renderer_text, "markup", 2, NULL, NULL);
	_tmp9_ = gtk_tree_view_get_column ((GtkTreeView*) self, 2);
	gtk_tree_view_column_set_alignment (_tmp9_, 0.0f);
	...
	index = 0;
	_tmp12_ = gtk_tree_view_get_columns ((GtkTreeView*) self);
	{
		GList* tvc_collection;
		GList* tvc_it;
		tvc_collection = _tmp12_;
		for (tvc_it = tvc_collection; tvc_it != NULL; tvc_it = tvc_it->next) {
			GtkTreeViewColumn* _tmp13_;
			GtkTreeViewColumn* tvc;
			_tmp13_ = _g_object_ref0 ((GtkTreeViewColumn*) tvc_it->data);
			tvc = _tmp13_;
			{
	...
	_tmp15_ = gtk_tree_view_get_selection ((GtkTreeView*) self);
	g_signal_connect_object (_tmp15_, "changed", (GCallback) _beat_box_side_tree_view_sideListSelectionChange_gtk_tree_selection_changed, self, 0);
	...
	self->priv->playlistMenu = _tmp17_;
	_tmp18_ = (GtkMenuItem*) gtk_menu_item_new_with_label ("New Playlist");
	_tmp19_ = g_object_ref_sink (_tmp18_);
	_g_object_unref0 (self->priv->playlistNew);
	self->priv->playlistNew = _tmp19_;
	_tmp20_ = (GtkMenuItem*) gtk_menu_item_new_with_label ("New Smart Playlist");
	_tmp21_ = g_object_ref_sink (_tmp20_);
	_g_object_unref0 (self->priv->smartPlaylistNew);
	self->priv->smartPlaylistNew = _tmp21_;
	_tmp22_ = (GtkMenuItem*) gtk_menu_item_new_with_label ("Edit");
	_tmp23_ = g_object_ref_sink (_tmp22_);
	_g_object_unref0 (self->priv->playlistEdit);
	self->priv->playlistEdit = _tmp23_;
	_tmp24_ = (GtkMenuItem*) gtk_menu_item_new_with_label ("Remove");
	_tmp25_ = g_object_ref_sink (_tmp24_);
	_g_object_unref0 (self->priv->playlistRemove);
	self->priv->playlistRemove = _tmp25_;
...
static void beat_box_side_tree_view_real_sideListSelectionChange (BeatBoxSideTreeView* self) {
	...
	if (BEAT_BOX_IS_MUSIC_TREE_VIEW (self->priv->current_widget)) {
		...
		_tmp32_ = g_strconcat (_tmp31_, fancy, NULL);
		_tmp33_ = g_strconcat (_tmp32_, ", ", NULL);
		_tmp34_ = g_strconcat (_tmp33_, fancy_size, NULL);
		beat_box_library_window_setStatusBarText (self->priv->lw, _tmp34_);
		_g_free0 (_tmp34_);
		_g_free0 (_tmp33_);
		_g_free0 (_tmp32_);
		_g_free0 (_tmp31_);
		...
		if (_tmp12_) {
			...
			if (_tmp21_) {
				...
				if (_gtk_tree_iter_equal (&iter, &self->playlists_queue_iter) == TRUE) {
					...
					_tmp23_ = _g_object_ref0 (BEAT_BOX_MUSIC_TREE_VIEW (w));
					mtv = _tmp23_;
					_tmp24_ = beat_box_library_manager_queue (self->priv->lm);
					_tmp25_ = _tmp24_;
					beat_box_music_tree_view_populateView (mtv, _tmp25_, FALSE);
					...
					if (_gtk_tree_iter_equal (&iter, &self->playlists_history_iter) == TRUE) {
						...
						_tmp26_ = _g_object_ref0 (BEAT_BOX_MUSIC_TREE_VIEW (w));
						mtv = _tmp26_;
						_tmp27_ = beat_box_library_manager_already_played (self->priv->lm);
						_tmp28_ = _tmp27_;
						beat_box_music_tree_view_populateView (mtv, (GeeCollection*) _tmp28_, FALSE);
						_g_object_unref0 (_tmp28_);
						_g_object_unref0 (mtv);
					} else {
						gboolean _tmp29_ = FALSE;
						if (_gtk_tree_iter_equal (&parent, &self->playlists_iter) == TRUE) {
							_tmp29_ = BEAT_BOX_IS_SMART_PLAYLIST (o);
						} else {
							_tmp29_ = FALSE;
						}
						if (_tmp29_) {
							BeatBoxMusicTreeView* _tmp30_;
							BeatBoxMusicTreeView* mtv;
							gint _tmp31_;
							GeeLinkedList* _tmp32_ = NULL;
							GeeLinkedList* _tmp33_;
							_tmp30_ = _g_object_ref0 (BEAT_BOX_MUSIC_TREE_VIEW (w));
							mtv = _tmp30_;
							_tmp31_ = beat_box_smart_playlist_get_rowid (BEAT_BOX_SMART_PLAYLIST (o));
							_tmp32_ = beat_box_library_manager_songs_from_smart_playlist (self->priv->lm, _tmp31_);
							_tmp33_ = _tmp32_;
							beat_box_music_tree_view_populateView (mtv, (GeeCollection*) _tmp33_, FALSE);
...
gboolean beat_box_side_tree_view_updateView (BeatBoxSideTreeView* self, GtkTreeModel* model, GtkTreePath* path, GtkTreeIter* item) {
	...
	if (w != NULL) {
		...
		if (_tmp1_) {
			...
			self->priv->current_widget = _tmp3_;
