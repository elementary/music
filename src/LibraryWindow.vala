...
public class BeatBox.LibraryWindow : LibraryWindowInterface, Gtk.ApplicationWindow {
    ...
    public Noise.Settings main_settings;
    public Noise.SavedState savedstate_settings;
    public Noise.Equalizer equalizer_settings;
    ...
    public LibraryWindow (BeatBox.Beatbox app) {
        ...
        if(lm.media_count() == 0 && main_settings.music_folder == "") {
            message("First run.\n");
        }
        else {
            lm.clearCurrent();
            ...
            if(lm.media_active && (double)(main_settings.last_media_position/(double)lm.media_info.media.length) > 0.90)
    ...
    public void build_ui() {
        ...
        set_default_size(savedstate_settings.window_width, savedstate_settings.window_height);
        ...
        view_container_hpaned.set_position((savedstate_settings.window_width - savedstate_settings.sidebar_width) - savedstate_settings.more_width);
        ...
        // Maximize window if necessary
        if (savedstate_settings.window_state == Noise.WindowState.MAXIMIZED)
            this.maximize ();
        else if (savedstate_settings.window_state == Noise.WindowState.FULLSCREEN)
            this.fullscreen ();
        ...
        info_panel.set_visible (savedstate_settings.more_visible);
...
// XXX FIXME TODO
    ...
    public override void destroy() {
        ...
        main_settings.last_media_position = (int)((double)lm.player.getPosition
        ()/1000000000);
        if(lm.media_active) {
